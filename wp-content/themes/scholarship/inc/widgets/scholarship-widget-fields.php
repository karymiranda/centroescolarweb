...
function scholarship_widgets_show_widget_field( $instance = '', $widget_field = '', $mt_widget_field_value = '' ) {
    ...
    switch ( $scholarship_widgets_field_type ) {
        ...
        ?>
            <p>
                ...
                <select name="<?php echo esc_attr( $instance->get_field_name( $scholarship_widgets_name ) ); ?>" id="<?php echo esc_attr( $instance->get_field_id( $scholarship_widgets_name ) ); ?>" class="widefat">
                    <?php foreach ( $scholarship_widgets_field_options as $select_option_name => $select_option_title ) { ?>
        ...
        case 'upload':
            ...
            <label for="<?php echo esc_attr( $instance->get_field_id( $scholarship_widgets_name ) ); ?>"><?php echo esc_html( $scholarship_widgets_title ); ?>:</label><br />
                ...
                <div class="actions clearfix">
                    <button type="button" class="button mt-delete-button align-left"><?php esc_html_e( 'Remove', 'scholarship' ); ?></button>
                    <button type="button" class="button mt-upload-button alignright"><?php esc_html_e( 'Select Image', 'scholarship' ); ?></button>
        ...
        <?php
            foreach ( $scholarship_widgets_field_options as $field_option_name => $field_option_title ) {
                if( isset( $mt_widget_field_value[$field_option_name] ) ) {
                    $mt_widget_field_value[$field_option_name] = 1;
                }else{
                    $mt_widget_field_value[$field_option_name] = 0;
                }
                
            ?>
                <div class="mt-single-checkbox">
                    <p>
                        <input id="<?php echo esc_attr( $instance->get_field_id( $field_option_name ) ); ?>" name="<?php echo esc_attr( $instance->get_field_name( $scholarship_widgets_name ).'['.$field_option_name.']' ); ?>" type="checkbox" value="1" <?php checked( '1', $mt_widget_field_value[$field_option_name] ); ?>/>
